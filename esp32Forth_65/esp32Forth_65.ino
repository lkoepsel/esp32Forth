/******************************************************************************/
/* esp32Forth, Version 6.5 : for Adafruit ESP32 Feather                       */
/******************************************************************************/
/* 05sep20wlk _65                                                             */
...
/* esp32Forth_62:397:26: error: format '%x' expects argument of type          */
...
// Added to allow for ?key2 to be a test in while word
// otherwise, ?key waits for buffer and doesn't exit
// copied from eForth328
void qrxt(void)
//  { while (Serial.available() == 0) {};
//    push Serial.read();
//    push -1; }
  { if (Serial.available() == 0) { push 0; }
   else { push Serial.read(); push 0xFFFF; }
  }
...
// new word for setting pin to input
void ipin(void)
{  WP=top; pop;
   pinMode(WP,INPUT);
}

// new word for setting pin to output
void opin(void)
{  WP=top; pop;
   pinMode(WP,OUTPUT);
}

// new word for digital write to pin
void dwrt(void)
{  WP=top; pop;
   digitalWrite(WP,top);
   pop;
...
// new word for a digital read on pin
void dread(void)
{  top= (int) digitalRead(top); }
...
// new word for setting pin to input pullup
void inpup(void)
{  WP=top; pop;
   pinMode(WP,INPUT_PULLUP);
}

// new word for setting pin to input pulldown
void inpdn(void)
{  WP=top; pop;
...
//digitalWrite(2, HIGH);   // turn the LED2 on 
  ...
  HEADER(4,"IPIN");
  int IPIN=CODE(4,as_ipin,as_next,0,0);
  HEADER(4,"OPIN");
  int OPIN=CODE(4,as_opin,as_next,0,0);
  HEADER(4,"DWRT");
  int DWRT=CODE(4,as_dwrt,as_next,0,0);
  HEADER(5,"DREAD");
  int DREAD=CODE(4,as_dread,as_next,0,0);
  HEADER(5,"GTIME");
  int GTIME=CODE(4,as_gtime,as_next,0,0);
  HEADER(5,"INPUP");
  int INPUP=CODE(4,as_inpup,as_next,0,0);
  HEADER(5,"INPDN");
  ...
  HEADER(4,"P0IN");
// changed P0IN to leave the value on the stack
//  int P0IN=COLON(5,DOLIT,0x3FF4403C,PEEK,DOT,EXITT);
  int P0IN=COLON(4,DOLIT,0x3FF4403C,PEEK,EXITT);
  HEADER(4,"P1IN");
...
// compile \data\load.txt  
  ...
  if(file) {
    ...
    WP = 0x184;
    evaluate();
    Serial.println(" Done latest loading."); 
    file.close();
    SPIFFS.end();
  }
}
